...
export default function LecturersPage() {
  ...
  useEffect(() => {
    async function fetchLecturers() {
      try {
        const res = await api.get("/users", { params: { role: "TUTOR" } })
  ...
  return (
    <DashboardShell breadcrumbs={[{ title: "Dashboard", href: "/dashboard" }, { title: "Users", href: "/dashboard/users" }, { title: "Lecturers" }]}>
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Lecturers</h1>
        <p className="text-sm text-muted-foreground mt-1">All teaching staff</p>
      </div>
      ...
      {!loading && users.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">No lecturers found</p>
        </div>
      )}
    </DashboardShell>
  )
